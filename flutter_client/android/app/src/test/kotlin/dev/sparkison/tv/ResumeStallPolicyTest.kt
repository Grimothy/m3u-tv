...
import dev.sparkison.tv.ResumeStallPolicy.Verdict
...
class ResumeStallPolicyTest {
    ...
    fun windowFloorsAtDefaultForNormalFrameRates() {
        // 24fps -> 4 x 42ms = 168ms, floored to 1000ms
        assertEquals(1000L, ResumeStallPolicy.checkWindowMs(formatFps = 24f, speed = 1f))
        assertEquals(1000L, ResumeStallPolicy.checkWindowMs(formatFps = 60f, speed = 1f))
    }

    @Test
    fun windowScalesUpForLowFrameRates() {
        // 2fps -> 4 x 500ms = 2000ms
        assertEquals(2000L, ResumeStallPolicy.checkWindowMs(formatFps = 2f, speed = 1f))
    }

    @Test
    fun windowScalesUpForSlowedPlayback() {
        // 8fps at 0.25x speed -> effective 2fps -> 2000ms
        assertEquals(2000L, ResumeStallPolicy.checkWindowMs(formatFps = 8f, speed = 0.25f))
        // fast playback shrinks the interval; still floored
        assertEquals(1000L, ResumeStallPolicy.checkWindowMs(formatFps = 24f, speed = 2f))
    }

    @Test
    fun windowFallsBackToDefaultFpsWhenFormatDoesNotReportOne() {
        assertEquals(1000L, ResumeStallPolicy.checkWindowMs(formatFps = null, speed = 1f))
        assertEquals(1000L, ResumeStallPolicy.checkWindowMs(formatFps = -1f, speed = 1f))
    }
    ...
    fun unknownDurationSkipsEofGuardAndStalls() {
        // Live TV has no known duration (C.TIME_UNSET) and must not suppress detection --
        // this is the exact case reported in #291.
        assertEquals(
            Verdict.STALLED,
            ResumeStallPolicy.evaluate(
                baselineFrames = 100,
